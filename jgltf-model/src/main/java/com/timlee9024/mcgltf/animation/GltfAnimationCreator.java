...
public final class GltfAnimationCreator {
	...
	public static List<InterpolatedChannel> createGltfAnimation(AnimationModel animationModel) {
		...
		for(Channel channel : channels) {
			...
			{
				Core.warn("Input data is not an AccessorFloatData, but "
						+ inputData.getClass());
				continue;
			}
			...
			{
				Core.warn("Output data is not an AccessorFloatData, but "
						+ outputData.getClass());
				continue;
			}
			...
			case "translation":
				...
				case CUBICSPLINE:
					...
					break;
				default:
					Core.warn("Interpolation type not supported: " + interpolation);
					break;
				}
				break;
			case "rotation":
			...
			case "weights":
				...
				break;
			default:
				Core.warn("Animation channel target path must be "
