...
import com.timlee9024.mcgltf.AccessorDataUtils;
import com.timlee9024.mcgltf.AccessorModelCreation;
...
public class RenderedGltfModel {
	...
	protected static final Runnable vanillaDefaultMaterialCommand = () -> {
		GL11.glBindTexture(GL11.GL_TEXTURE_2D, Core.getInstance().getDefaultColorMap());
	...
	protected static final Runnable shaderModDefaultMaterialCommand = () -> {
		...
		GL11.glBindTexture(GL11.GL_TEXTURE_2D, Core.getInstance().getDefaultNormalMap());
		GL13.glActiveTexture(SPECULAR_MAP_INDEX);
		GL11.glBindTexture(GL11.GL_TEXTURE_2D, Core.getInstance().getDefaultSpecularMap());
		GL20.glVertexAttrib4f(vaColor, 1.0F, 1.0F, 1.0F, 1.0F);
		GL11.glEnable(GL11.GL_CULL_FACE);
	};
		...
		public void initMaterialCommand(List<Runnable> gltfRenderData, RenderedGltfModel renderedModel, MaterialModel materialModel) {
			...
			if(materialModel instanceof MaterialModelV2) {
				...
				if(baseColorTexture == null) {
					...
					else colorMap = Core.getInstance().getDefaultColorMap();
				}
				...
				if(normalTexture == null) {
					...
					else normalMap = Core.getInstance().getDefaultNormalMap();
				}
				...
				if(specularTexture == null) {
					...
					else specularMap = Core.getInstance().getDefaultSpecularMap();
				}
				...
				colorMap = baseColorTexture == null ? Core.getInstance().getDefaultColorMap() : renderedModel.obtainGlTexture(gltfRenderData, textureModels.get(baseColorTexture.index));
				normalMap = normalTexture == null ? Core.getInstance().getDefaultNormalMap() : renderedModel.obtainGlTexture(gltfRenderData, textureModels.get(normalTexture.index));
				specularMap = specularTexture == null ? Core.getInstance().getDefaultSpecularMap() : renderedModel.obtainGlTexture(gltfRenderData, textureModels.get(specularTexture.index));
	...
	public void bindArrayBufferViewModel(List<Runnable> gltfRenderData, BufferViewModel bufferViewModel) {
		...
		if(glBufferView == null) {
			int glBufferViewNew = GL15.glGenBuffers();
	...
	public int obtainGlTexture(List<Runnable> gltfRenderData, TextureModel textureModel) {
		...
		if(glTexture == null) {
			...
			if (pixelData == null)
			{
				Core.warn("Could not extract pixel data from image");
